...
d3.csv("../static/data/StatinConsuming_PersonIDs.csv", function(statin) {
  console.log(statin);
d3.csv("../static/data/t2dcomorbids_lineage_4thJan.csv", function(comorbids) {
...
*/
  ...
  xscale.domain(dimensions =dims.filter(function(k) {
    ...
    return (k!=="") && (k!=="person_id") && (k!=="gender") && (yscale[k] =  (k ==="statin") ? d3.scale.ordinal().domain(["S", "NS"]).range([h, 0]) :
              ...
              .range([h, 0])); 
      /*return (_.isNumber(data[0][k])) && (k!=="")  && (yscale[k] =  (k ==="statin") ? d3.scale.ordinal().domain(["S", "NS"]).range([h, 0]) :
              d3.scale.linear()
              //.domain([0,2000])
              .domain(d3.extent(data, function(p) { return +p[k]; }))
              .range([h, 0]));*/
...
function create_unique_conditions(precs){
      ...
      for(var p in precs){
        for(key in precs[p]['concepts'])
        {
          ...
          var concept = precs[p]['concepts'][key]['concept_name'];
          ...
          unique_condition_ids[precs[p]['concepts'][key]['concept_name']] =0;
        }
      }

      // makes an array of dictionaries, with each condition and its size, i.e., frequency 
